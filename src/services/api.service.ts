 ...
 * This provides a basic segmentation when the API is unavailable
 */
...
export async function generateText(
  ...
  // Convert frontend segments to backend format
...
/**
 * Get text-to-text explanation mappings via backend API
 */
...
/**
 * Get text-to-image explanation mappings via backend API
 */
export async function explainImage(
  ...
  // Transform heatmap regions to point-based heatmap for frontend
  const mappings = segments.map(seg => {
    ...
    regionData.forEach(region => {
      // Convert region to multiple points for gradient effect
